      ...
      <div className="relative w-full px-4 sm:px-6 lg:px-8 py-20 lg:py-32">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="text-center lg:text-left">
            ...
            >
              Track Attendance,<br />
              <span className="gradient-text">Manage Workforce</span>,<br />
              Boost Productivity.
            </motion.h1>
            ...
            >
              Automate attendance tracking, leave approvals, payroll integration, and workforce analytics with one intelligent platform.
            </motion.p>
          ...
          >
            <div className="relative w-full aspect-[4/3]">
              ...
              <div className="relative w-full h-full glass rounded-3xl p-4 sm:p-6 shadow-2xl shadow-primary-500/10 border border-white/5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="flex-1 h-6 bg-surface-light/50 rounded-lg flex items-center px-3">
                    <span className="text-xs text-muted">dashboard.attendpro.com</span>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-3 mb-3">
                  {stats.map((stat) => (
                    <div key={stat.label} className="bg-surface/80 rounded-xl p-3 border border-white/5">
                ...
                <div className="bg-surface/80 rounded-xl p-3 border border-white/5">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-semibold text-light">Today's Activity</span>
                    <span className="text-xs text-primary-400 font-medium">View All</span>
                  </div>
                  {employees.map((emp, i) => (
                    <div key={i} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-light">{emp.name}</p>
                          <p className="text-xs text-muted">{emp.time}</p>
                        </div>
                      </div>
                      <span className={`text-xs font-medium ${emp.color}`}>{emp.status}</span>
                    </div>
                  ))}
                </div>
              </div>
